...
目前Kubernetes最新版为v1.24，这里涉及到Kubernetes底层运行时的问题，官方通告从Kubernetes1.24开始，Dockershim将会被移除，也就是从v1.24开始便不再支持Docker引擎作为底层运行时，而转为使用containerd作为底层运行时。这一决定其实早在v1.20开始就宣布的，最后决定从v1.24开始才正式移除。所以v1.22是最后一个支持Docker运行时的Kubernetes版本，本文还是以Docker作为底层来进行学习。

> Kubernetes关于弃用Dockershim的解释：https://kubernetes.io/zh/blog/2022/02/17/dockershim-faq/
>
> 华为云关于K8s和Docker兼容性说明：https://support.huaweicloud.com/twp-kunpengcpfs/topic_0000001119261466.html
>
> 阿里云Docker与Containerd的区别：https://help.aliyun.com/document_detail/160313.html?spm=5176.2020520152.help.dexternal.48c716ddPahqsZ
...
![部署演进](https://v1-22.docs.kubernetes.io/images/docs/Container_Evolution.svg)

**传统部署时代：**互联网早期，会直接将应用程序部署在物理机上
...
**虚拟化部署时代：**可以在一台物理机上运行多个虚拟机，每个虚拟机都是独立的一个环境
...
![image-20200524150339551](http://img.chenxie.net/md-img/image-20200524150339551.png)
...
![image-20220308112104283](http://img.chenxie.net/md-img/image-20220308112104283.png)
...
> 官方对于Kubernetes组件的解释：https://v1-22.docs.kubernetes.io/zh/docs/concepts/overview/components/
>
> ![Kubernetes 组件](https://v1-22.docs.kubernetes.io/images/docs/components-of-kubernetes.svg)



一个kubernetes集群主要是由**控制节点(master)**、**工作节点(node)**构成，每个节点上都会安装不同的组件。

**master：集群的控制平面组件（Control Plane Components），负责集群的决策 ( 管理 )**
...
>
> **容器运行时** : Kubernetes 支持多个容器运行环境: [Docker](https://v1-22.docs.kubernetes.io/zh/docs/reference/kubectl/docker-cli-to-kubectl/)、 [containerd](https://containerd.io/docs/)、[CRI-O](https://cri-o.io/#what-is-cri-o) 以及任何实现 [Kubernetes CRI (容器运行环境接口)](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-node/container-runtime-interface.md)的环境

![image-20200406184656917](http://img.chenxie.net/md-img/image-20200406184656917.png)
...
![image-20200404094800622](http://img.chenxie.net/md-img/image-20200404094800622.png)
...
![image-20200406225334627](http://img.chenxie.net/md-img/image-20200406225334627.png)
...
# 查看pod的详细信息
kubectl get pod -o wide  # 所有pod
kubectl get pod pod_name -o wide  # 指定pod
kubectl get pod pod_name -o json  # 以json形式显示
kubectl get pod pod_name -o yaml  # 以yaml形式显示
```
...
# 在此namespace下创建并运行一个nginx的Pod叫nginx
[root@master ~]# kubectl run nginx --image=nginx -n dev
pod/nginx created
...
上面文件如果手写的话不好写还容易出错，可以这样做：

生成创建dev命名空间的yaml文件内容：

```bash
[root@master ~]# kubectl create ns dev --dry-run=client -o yaml > ns-dev.yaml
[root@master ~]# 
...
生成创建nginx pod的yaml文件内容：

```bash
[root@master ~]# kubectl run nginx --image=nginx -n dev --dry-run=client -o yaml > nginx3.yaml
[root@master ~]# 
[root@master ~]# cat nginx3.yaml 
apiVersion: v1
kind: Pod
metadata:
  creationTimestamp: null
...
我们可以单独的执行生成的这两个yaml文件，也可以将两个内容整理到一个yaml文件中，整理之后就成了我们需要的内容：
...
![image-20200407100850484](http://img.chenxie.net/md-img/image-20200407100850484.png)
...
![image-20200407121501907](http://img.chenxie.net/md-img/image-20200407121501907.png)
...
Namespace:    dev
Priority:     0
Node:         node2/192.168.92.157
Start Time:   Thu, 10 Mar 2022 23:56:12 +0800
Labels:       run=nginx
Annotations:  <none>
Status:       Running
IP:           10.244.2.8
...
Containers:
  nginx:
    Container ID:   docker://e07d24e6bbca4436590fe4331883cf4cb2c13260ce22ae246a4f526b0ace692c
    Image:          nginx:latest
    Image ID:       docker-pullable://nginx@sha256:0d17b565c37bcbd895e9d92315a05c1c3c9a29f762b011a10c54a66cd53c9b31
    Port:           80/TCP
    Host Port:      0/TCP
    State:          Running
      Started:      Thu, 10 Mar 2022 23:56:29 +0800
    Ready:          True
    Restart Count:  0
    Environment:    <none>
    Mounts:
      /var/run/secrets/kubernetes.io/serviceaccount from kube-api-access-jk22w (ro)
Conditions:
  Type              Status
  Initialized       True 
  Ready             True 
...
Events:
  ...
  ----    ------     ----  ----               -------
  Normal  Scheduled  53s   default-scheduler  Successfully assigned dev/nginx to node2
  Normal  Pulling    52s   kubelet            Pulling image "nginx:latest"
  Normal  Pulled     36s   kubelet            Successfully pulled image "nginx:latest" in 15.565924535s
...
nginx   1/1     Running   0          3m6s   10.244.2.8   node2   <none>           <none>
#访问POD
...
--------- 以下在老版本k8s中有效，当前v1.22中 kubectl run 不会创建deploy，固无效----------
...
nginx   1/1     Running   0          68s   version=1.0
...
NAME    READY   STATUS    RESTARTS   AGE     LABELS
nginx   1/1     Running   0          3m42s   version=2.0
...
[root@master ~]# kubectl get pod nginx -n dev --show-labels
NAME    READY   STATUS    RESTARTS   AGE     LABELS
nginx   1/1     Running   0          4m36s   <none>  # 无标签了
```
...
NAME    READY   STATUS    RESTARTS   AGE   LABELS
nginx   1/1     Running   0          14s   env=test,version=3.0
```
...
![image-20200408193950807](http://img.chenxie.net/md-img/image-20200408193950807.png)
...
![image-20200408194716912](http://img.chenxie.net/md-img/image-20200408194716912.png)
...
```shell
# 暴露Service
# --port=80 表示service暴露的端口
# --target-port=80 表示pod的端口
...
svc-nginx1   ClusterIP   10.107.188.215   <none>        80/TCP    16s   app=nginx

# 这里产生了一个CLUSTER-IP，这就是service的IP，在Service的生命周期中，这个地址是不会变动的
# 可以通过这个IP访问当前service对应的POD
[root@master ~]# curl 10.107.188.215:80
<!DOCTYPE html>
<html>
<head>
<title>Welcome to nginx!</title>
...
NAME         TYPE       CLUSTER-IP       EXTERNAL-IP   PORT(S)        AGE   SELECTOR
svc-nginx2   NodePort   10.109.159.124   <none>        80:31399/TCP   16s   app=nginx

# 接下来就可以通过集群外的主机访问 节点IP:31399访问服务了
# 例如在的电脑主机上通过浏览器访问下面的地址
[root@master ~]# curl http://192.168.92.163:31399
<!DOCTYPE html>
<html>
<head>
<title>Welcome to nginx!</title>
...
spec:
  ...
  selector:
    run: nginx
  type: ClusterIP   # 这里写了ClusterIP，上面clusterIP可以不写
```
...
![image-20200407121501907](http://img.chenxie.net/md-img/image-20200407121501907-1626781151898.png)
...
![image-20210617223823675](http://img.chenxie.net/md-img/image-20210617223823675-1626781695411.png)
...
# 可以通过describe查看内部的详情
# 此时已经运行起来了一个基本的Pod，虽然它暂时有问题，原因在下面 启动命令章节解释
...
![image-20210617223923659](http://img.chenxie.net/md-img/image-20210617223923659.png)
...
![image-20210617224457945](http://img.chenxie.net/md-img/image-20210617224457945.png)
...
![image-20200412111402706](http://img.chenxie.net/md-img/image-20200412111402706-1626782188724.png)
...
6. apiServer将接收到的pod状态信息存入etcd中
   ...
   ![image-20200406184656917](http://img.chenxie.net/md-img/image-20200406184656917-1626782168787.png)
...
为了简化测试，事先假设mysql`(192.168.92.201)`和redis`(192.168.92.202)`服务器的地址
...
spec:
  ...
  - name: test-mysql
    image: busybox:1.30
    command: ['sh', '-c', 'until ping 192.168.92.201 -c 1 ; do echo waiting for mysql...; sleep 2; done;']
  - name: test-redis
    image: busybox:1.30
    command: ['sh', '-c', 'until ping 192.168.92.202 -c 1 ; do echo waiting for reids...; sleep 2; done;']
```
...
[root@k8s-master01 ~]# ifconfig ens33:1 192.168.92.201 netmask 255.255.255.0 up
[root@k8s-master01 ~]# ifconfig ens33:2 192.168.92.202 netmask 255.255.255.0 up
```
...
![image-20200605021606508](http://img.chenxie.net/md-img/image-20200605021831545.png)
...
![image-20200514095913741](http://img.chenxie.net/md-img/image-20200514095913741.png)
...
![img](http://img.chenxie.net/md-img/image-20200612005334159.png)
...
![img](http://img.chenxie.net/md-img/image-20200612005524778.png)
...
![img](http://img.chenxie.net/md-img/image-20200416140251491.png)
...
![img](http://img.chenxie.net/md-img/image-20200608155858271.png)
...
![image-20220510234802587](http://img.chenxie.net/md-img/image-20220510234802587.png)
...
[root@master ~]# kubectl get pod -n kube-system
NAME                              READY   STATUS    RESTARTS         AGE
metrics-server-6f796dd456-kb5tq   1/1     Running   0                7m55s

# 使用 kubectl top node 查看资源使用情况
[root@master ~]# kubectl top node
NAME     CPU(cores)   CPU%   MEMORY(bytes)   MEMORY%   
master   827m         41%    1110Mi          64%       
node1    274m         13%    531Mi           30%       
node2    344m         17%    536Mi           31% 

[root@master ~]# kubectl top pod -n kube-system
NAME                              CPU(cores)   MEMORY(bytes)   
coredns-7d89d9b6b8-9qqt8          12m          15Mi            
coredns-7d89d9b6b8-cpbt2          8m           33Mi            
etcd-master                       92m          84Mi            
kube-apiserver-master             281m         303Mi           
kube-controller-manager-master    116m         87Mi            
kube-flannel-ds-h79ks             11m          24Mi            
kube-flannel-ds-vqc5s             11m          31Mi            
kube-flannel-ds-zjdgv             15m          23Mi            
kube-proxy-md2zs                  6m           26Mi            
kube-proxy-rq5sz                  5m           18Mi            
kube-proxy-x2xxc                  1m           20Mi            
kube-scheduler-master             20m          39Mi            
metrics-server-6f796dd456-kb5tq   19m          14Mi
...
NAME                        READY   STATUS             RESTARTS         AGE
pod/nginx-cd5f9b74c-9rtpn   1/1     Running            0                76s

NAME            TYPE       CLUSTER-IP      EXTERNAL-IP   PORT(S)        AGE
service/nginx   NodePort   10.110.85.226   <none>        80:30782/TCP   49s
```
...
pc-hpa   Deployment/nginx   0%/3%     1         10        1          18s
```

**4 测试**

使用压测工具对service地址 192.168.92.163:30782 进行压测，然后通过控制台查看hpa和pod的变化
...
![img](http://img.chenxie.net/md-img/image-20200612010223537.png)
...
[root@master ~]# kubectl get pod -n dev -o wide
NAME                    READY   STATUS             RESTARTS         AGE     IP            NODE    NOMINATED 
pc-daemonset-489ch      1/1     Running            0                77s     10.244.1.37   node1   <none>     
pc-daemonset-5c52r      1/1     Running            0                77s     10.244.2.34   node2   <none>     
...
![img](http://img.chenxie.net/md-img/image-20200618213054113.png)
...
![img](http://img.chenxie.net/md-img/image-20200618213149531.png)
...
本章节主要介绍kubernetes的流量负载组件：Service 和 ingress
...
![img](http://img.chenxie.net/md-img/image-20200408194716912-1626783758946.png)

Service在很多情况下只是一个概念，真正起作用的其实是kube-proxy服务进程，每个Node节点上都运行着一个kube-proxy服务进程。当创建Service的时候会通过api-server向etcd写入创建的service的信息，而kube-proxy会基于监听的机制发现这种Service的变动，然后**它会将最新的Service信息转换成对应的访问规则**。

![img](http://img.chenxie.net/md-img/image-20200509121254425.png)
...
![img](http://img.chenxie.net/md-img/image-20200509151424280.png)

**iptables 模式**

iptables模式下，kube-proxy为service后端的每个Pod创建对应的iptables规则，直接将发向Cluster IP的请求重定向到一个Pod IP。  该模式下kube-proxy不承担四层负责均衡器的角色，只负责创建iptables规则。该模式的优点是较userspace模式效率更高，但不能提供灵活的LB策略，当后端Pod不可用时也无法进行重试。

![img](http://img.chenxie.net/md-img/image-20200509152947714.png)

**ipvs 模式**

ipvs模式和iptables类似，kube-proxy监控Pod的变化并创建相应的ipvs规则。ipvs相对iptables转发效率更高。除此以外，ipvs支持更多的LB算法。

![img](http://img.chenxie.net/md-img/image-20200509153731363.png)
...
![image-20200509191917069](http://img.chenxie.net/md-img/image-20200509191917069.png)
...
![img](http://img.chenxie.net/md-img/image-20200620175731338.png)
...
![img](http://img.chenxie.net/md-img/image-20200510103945494.png)

### 7.3.6 ExternalName类型的Service

ExternalName类型的Service用于引入集群外部的服务，它通过`externalName`属性指定外部一个服务的地址，然后在集群内部访问此service就可以访问到外部的服务了。

![img](http://img.chenxie.net/md-img/image-20200510113311209.png)
...
![img](http://img.chenxie.net/md-img/image-20200623092808049.png)
...
![img](http://img.chenxie.net/md-img/image-20200516112704764.png)
...
![img](http://img.chenxie.net/md-img/image-20200516102419998.png)
...
![img](http://img.chenxie.net/md-img/image-20200413174713773.png)
...
![img](http://img.chenxie.net/md-img/image-20200413214031331.png)
...
![img](http://img.chenxie.net/md-img/image-20200413215133559.png)
...
![img](http://img.chenxie.net/md-img/image-20200514194111567.png)
...
![img](http://img.chenxie.net/md-img/image-20200515002806726.png)
...
![img](http://img.chenxie.net/md-img/image-20200520102949189.png)
...
![img](http://img.chenxie.net/md-img/image-20200520103942580.png)
...
![img](http://img.chenxie.net/md-img/image-20200518211037434.png)
...
![img](http://img.chenxie.net/md-img/image-20200519181209566.png)
...
# 修改kubernetes-dashboard的Service类型为NodePort
spec:
  type: NodePort  # 新增
  ports:
    - port: 443
...
NAME                                            READY   STATUS    RESTARTS   AGE
pod/dashboard-metrics-scraper-c45b7869d-w9n4r   1/1     Running   0          2m47s
pod/kubernetes-dashboard-79b5779bf4-qvpgf       1/1     Running   0          2m47s

NAME                                TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)    AGE
service/dashboard-metrics-scraper   ClusterIP   10.103.120.210   <none>        8000/TCP   2m48s
service/kubernetes-dashboard        ClusterIP   10.99.108.67     <none>        443/TCP    2m50s
```
...
dashboard-admin-token-kt2zq        kubernetes.io/service-account-token   3      57s

[root@master ~]# kubectl describe secrets dashboard-admin-token-kt2zq -n kubernetes-dashboard
Name:         dashboard-admin-token-kt2zq
Namespace:    kubernetes-dashboard
Labels:       <none>
Annotations:  kubernetes.io/service-account.name: dashboard-admin
              kubernetes.io/service-account.uid: a030a9ba-7163-4291-b7d4-4ba4cfbef1c7
...
====
namespace:  20 bytes
token:      eyJhbGciOiJSUzI1NiIsImtpZCI6IkctSWhZZWl5RXI0cGxLNWVYemxzOXZfWUpKSU1YLTVuYWRLcEYwVmJyOTgifQ..E7i1Tb898OLA6Bicbvpjj1Xuy3Xh4QICK5D5MsyjFQ-dL5_6TfjLHKEu_LPkVt7rXzobqB-Bqps814IjhcNuebah0dJj5iUTljryw_6yTo6brPBC4EZJzUTaB9_sp5vbWmrHht61oLTtLxhymdZZtuGVY_7aM2vBSZnxvdlhwpD5sW68ukFhi59xDtTR571QSy6H49czwOn2rHjwl4EvI2rhydSdCNjA9xNy_5YftOm6q9s06vwccSpdw-c0nllEAnlbhQFhnuMhyWOl6uPPgvYAHIUN_PjrU0e43vvYF__yrhS8_aEs6lqUwQd2tHQSSgB1QqohlUhUuhV-hcn1og
ca.crt:     1099 bytes
```
...
![image-20200520144548997](http://img.chenxie.net/md-img/image-20200520144548997.png)

出现下面的页面代表成功

![image-20200520144959353](http://img.chenxie.net/md-img/image-20200520144959353.png)
...
![img](http://img.chenxie.net/md-img/image-20200520154628679.png)

**扩缩容**

在`Deployment`上点击`规模`，然后指定`目标副本数量`，点击确定

![img](http://img.chenxie.net/md-img/image-20200520162605102.png)

**编辑**

在`Deployment`上点击`编辑`，然后修改`yaml文件`，点击确定

![image-20200520163253644](http://img.chenxie.net/md-img/image-20200520163253644.png)

**查看Pod**

点击`Pods`, 查看pods列表

![img](http://img.chenxie.net/md-img/image-20200520163552110.png)

**操作Pod**

选中某个Pod，可以对其执行日志（logs）、进入执行（exec）、编辑、删除操作

![img](http://img.chenxie.net/md-img/image-20200520163832827.png)
